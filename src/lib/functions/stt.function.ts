...
import { shouldUseDeskifyAPI } from "./deskify.api";

// Deskify STT function
async function fetchDeskifySTT(audio: File | Blob): Promise<string> {
  try {
    ...
    return `Deskify STT Error: ${errorMessage}`;
  }
}
...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    // Check if we should use Deskify API instead
    const useDeskifyAPI = await shouldUseDeskifyAPI();
    if (useDeskifyAPI) {
      return await fetchDeskifySTT(audio);
    }

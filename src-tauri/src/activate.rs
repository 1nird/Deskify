...
fn get_payment_endpoint() -> Result<String, String> {
    ...
    match option_env!("PAYMENT_ENDPOINT") {
        ...
        None => Ok("https://api.cluely.ai".to_string())
    }
}
...
struct SecureStorage {
    ...
    selected_deskify_model: Option<String>,
}
...
pub async fn secure_storage_save(app: AppHandle, items: Vec<StorageItem>) -> Result<(), String> {
    ...
    for item in items {
        match item.key.as_str() {
            "deskify_license_key" => storage.license_key = Some(item.value),
            "deskify_instance_id" => storage.instance_id = Some(item.value),
            "selected_deskify_model" => storage.selected_deskify_model = Some(item.value),
...
pub async fn secure_storage_get(app: AppHandle) -> Result<StorageResult, String> {
    ...
    Ok(StorageResult {
        ...
        selected_deskify_model: storage.selected_deskify_model,
    })
}
...
pub async fn secure_storage_remove(app: AppHandle, keys: Vec<String>) -> Result<(), String> {
    ...
    for key in keys {
        match key.as_str() {
            "deskify_license_key" => storage.license_key = None,
            "deskify_instance_id" => storage.instance_id = None,
            "selected_deskify_model" => storage.selected_deskify_model = None,
...
pub async fn activate_license_api(
    ...
    let _machine_id: String = app.machine_uid().get_machine_uid().unwrap().id.unwrap();
    let _app_version: String = env!("CARGO_PKG_VERSION").to_string();

    // Bypass external payment endpoint and fake a successful local activation
    let activation_response = ActivationResponse {
        ...
        instance: Some(InstanceInfo {
            id: instance_name,
            name: "local-deskify-instance".to_string(),
...
pub async fn validate_license_api(app: AppHandle) -> Result<ValidateResponse, String> {
    ...
    let validate_response = ValidateResponse {
        is_active: true,
        last_validated_at: Some("2026-05-09T00:00:00Z".to_string()),
        is_dev_license: true,
    };

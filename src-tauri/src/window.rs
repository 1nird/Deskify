...
/// Hide the dashboard window if it is visible (no-op if already hidden).
#[tauri::command]
pub fn close_dashboard(app: tauri::AppHandle) -> Result<(), String> {
...
/// Tauri command to set content protection (hides window from screen capture)
#[tauri::command]
pub fn set_content_protected(app: tauri::AppHandle, protected: bool) -> Result<(), String> {
    if let Some(window) = app.get_webview_window("main") {
        window
            .set_content_protected(protected)
            .map_err(|e| format!("Failed to set content protection for main: {}", e))?;
    }
    if let Some(dashboard) = app.get_webview_window("dashboard") {
        dashboard
            .set_content_protected(protected)
            .map_err(|e| format!("Failed to set content protection for dashboard: {}", e))?;
    }
    Ok(())
}

/// Toggle whether the main overlay window intercepts OS-level mouse events.
/// When ignore=true the window is fully click-through (transparent to cursor),
/// fixing the bug where Deskify blocks scrolling and clicking on underlying apps.
#[tauri::command]
...
pub fn show_dashboard_window<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    if let Some(dashboard_window) = app.get_webview_window("dashboard") {
        // Restore from minimized / obscured state before show (Windows reliability)

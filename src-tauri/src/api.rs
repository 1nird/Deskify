...
#[derive(Debug, Serialize, Deserialize)]
pub struct DeskifyPromptsResponse {
    prompts: Vec<DeskifyPrompt>,
    total: i32,
...
// Fetch Deskify Prompts API
#[tauri::command]
pub async fn fetch_prompts(app: tauri::AppHandle) -> Result<DeskifyPromptsResponse, String> {
    let app_endpoint = get_app_endpoint()?;
    let api_access_key = get_api_access_key()?;
    ...
    let prompts_response: DeskifyPromptsResponse = response
        .json()
        .await

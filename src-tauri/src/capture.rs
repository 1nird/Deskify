...
pub async fn capture_to_base64(window: tauri::WebviewWindow) -> Result<String, String> {
    ...
    if let Err(e) = window.hide() {
        eprintln!("Failed to hide window before capture: {}", e);
    }
    ...
    let capture_result = tauri::async_runtime::spawn_blocking(move || {
        ...
        while retry_count < 3 {
            match Monitor::all() {
                ...
                Ok(_) => {
                    eprintln!("No monitors found, retry {}/3...", retry_count + 1);
                }
                Err(e) => {
                    eprintln!("Error getting monitors: {}, retry {}/3...", e, retry_count + 1);
                }
            }
            retry_count += 1;

    ...
    #[cfg(target_os = "windows")]
    {
        ...
        *is_hidden = !*is_hidden;
        ...
        if !*is_hidden {
            ...
            // Actually hide the window at the OS level on Windows so it stops capturing clicks!
...
pub fn update_shortcuts<R: Runtime>(
    ...
    for (action_id, binding) in &config.bindings {
        if binding.enabled && !binding.key.is_empty() {
            ...
            if action_id == "scroll_chat" {
                let modifiers = binding.key.trim();
                if modifiers.is_empty() {
                    continue;
                }

                let arrow_keys = vec!["up", "down"];
                for arrow in arrow_keys {
                    let full_key = format!("{}+{}", modifiers, arrow);
                    match full_key.parse::<Shortcut>() {
                        Ok(shortcut) => {
                            let direction_action_id = format!("scroll_chat_{}", arrow);
                            shortcuts_to_register.push((direction_action_id, full_key, shortcut));
                        }
                        Err(e) => {
                            eprintln!("Invalid shortcut '{}' for scroll_chat: {}", full_key, e);
                            return Err(format!(
                                "Invalid shortcut '{}' for scroll_chat: {}",
                                full_key, e
                            ));
                        }
                    }
